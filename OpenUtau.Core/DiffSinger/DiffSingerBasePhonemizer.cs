...
namespace OpenUtau.Core.DiffSinger
{
    public abstract class DiffSingerBasePhonemizer : MachineLearningPhonemizer
    {
        ...
        protected override void ProcessPart(Note[][] phrase) {
            ...
            //Group-aware duration models derive the phone-to-group map from word_div
            //and take every group's frame budget from word_dur. Both tensors are
            //already built for the linguistic encoder above, so pass them on when
            //(and only when) the duration graph declares them; older duration
            //models keep working unchanged because they do not declare them.
