...
namespace OpenUtau.Core.DiffSinger
{
    public abstract class DiffSingerBasePhonemizer : MachineLearningPhonemizer
    {
        ...
        protected virtual IG2p LoadG2p(string rootPath, bool useLangId = false)
        {
            ...
            foreach (var dictionaryName in dictionaryNames)
            {
                ...
                if (File.Exists(dictionaryPath))
                {
                    try
                    {
                        g2pBuilder.Load(File.ReadAllText(dictionaryPath));
                    }
                    catch (Exception e)
                    {
        ...
        List<phonemesPerNote> ProcessWord(Note[] notes, string[] symbols)
        {
            ...
            if (dsPhonemes.Length > 0 && isVowel.All(b => !b))
            {
                isStart[0] = true;
            }
            ...
            for (int i = 0; i < dsPhonemes.Length; i++)
            {
                ...
                {
                    wordPhonemes.Last().Phonemes.Add(dsPhonemes[i]);
                }
            }
            return wordPhonemes;
        }
        ...
        protected override void ProcessPart(Note[][] phrase)
        {
            ...
            var allSymbolsInPhrase = phrase.SelectMany(word => GetSymbols(word[0])).ToList();
            bool hasHeadSP = allSymbolsInPhrase.FirstOrDefault() == "SP";
            bool hasTailSP = allSymbolsInPhrase.LastOrDefault() == "SP";

            var phrasePhonemes = new List<phonemesPerNote>();
            var notePhIndex = new List<int>();

            // If user has NO head SP, we add one.
            if (!hasHeadSP)
            {
                phrasePhonemes.Add(new phonemesPerNote(-1, phrase[0][0].tone, new List<dsPhoneme> { new dsPhoneme("SP", GetSpeakerAtIndex(phrase[0][0], 0)) }));
                notePhIndex.Add(1);
            }
            else
            {
                phrasePhonemes.Add(new phonemesPerNote(-1, phrase[0][0].tone));
                notePhIndex.Add(0);
            }
            ...
            foreach (int wordIndex in Enumerable.Range(0, phrase.Length))
            {
                ...
                phrasePhonemes.Last().Phonemes.AddRange(wordPhonemes[0].Phonemes);
                phrasePhonemes.AddRange(wordPhonemes.Skip(1));
                notePhIndex.Add(notePhIndex.Last() + wordPhonemes.SelectMany(n => n.Phonemes).Count());
            }

            // If user has NO tail SP, we add one.
            if (!hasTailSP)
            {
                phrasePhonemes.Last().Phonemes.Add(new dsPhoneme("SP", GetSpeakerAtIndex(lastNote, 0)));
            }

            phrasePhonemes.Add(new phonemesPerNote(endTick, lastNote.tone));

            if (phrasePhonemes.Count > 1)
            {
                var firstPhonemeGroup = phrasePhonemes.FirstOrDefault(p => p.Phonemes.Any() && p.Position != -1);
                if (firstPhonemeGroup != null)
                {
                    phrasePhonemes[0].Position = timeAxis.MsPosToTickPos(
                       timeAxis.TickPosToMsPos(firstPhonemeGroup.Position) - padding
                   );
                }
                else
                {
                    phrasePhonemes[0].Position = timeAxis.MsPosToTickPos(startMs);
                }
            }
            else
            {
            ...
            int phonemeOffset = hasHeadSP ? 0 : 1;
            ...
            if (phAlignPoints.Any() && durationFrames.Count > 0)
            {
                int count = phAlignPoints[0].Item1 - phonemeOffset;
                if (count > 0 && phonemeOffset + count <= durationFrames.Count)
                {
                    List<double> alignGroup = durationFrames.GetRange(phonemeOffset, count);
            ...
            foreach (var pair in phAlignPoints.Zip(phAlignPoints.Skip(1), (a, b) => Tuple.Create(a, b)))
            {
                ...
                if (currAlignPoint.Item1 < durationFrames.Count)
                {
                    var count = nextAlignPoint.Item1 - currAlignPoint.Item1;
                    if (currAlignPoint.Item1 + count <= durationFrames.Count)
                    {
                        var alignGroup = durationFrames.GetRange(currAlignPoint.Item1, count);
                        if (alignGroup.Any())
                        {
            ...
            foreach (int wordIndex in Enumerable.Range(0, phrase.Length))
            {
                ...
                if (!wordFound[wordIndex]) { continue; }
                if (word[0].lyric.StartsWith("+")) { continue; }
