...
namespace OpenUtau.Core.DiffSinger{
    ...
    public class DsVariance : IDisposable{
        ...
        public VarianceResult Process(RenderPhrase phrase){
            ...
            //Variance Predictor
            var pitch = DiffSingerUtils.SampleCurve(phrase, phrase.pitches, 0, frameMs, totalFrames, headFramesForCurve, tailFramesForCurve, 
                x => x * 0.01).Select(f => (float)f).ToArray();
            var toneShift = DiffSingerUtils.SampleCurve(phrase, phrase.toneShift, 0, frameMs, totalFrames, headFramesForCurve, tailFramesForCurve,
                x => x * 0.01).Select(f => (float)f).ToArray();
